...
兄弟对 agent「自报完成」信任度低(2026-08-18 实锤:Phase D-r2 自报完成但漏跑 Step 4/5 纹理+vmd+snapshot;Phase C-r2 自报 snapshot 是 PR 造成但未实测)→ 对外断言(尤其 Blocking)必须附实测命令+输出(基线对比、grep 行号),不能凭代码阅读下结论。已 patch gts-auto §7.4.1 + gts-code-review。
§
