...
- `gts-dev-feat` — **跳过 M 阶段**（M-1~M-4 全跳过），直接进 C3
- `gts-dev-fix` — **跳过 M 阶段**（M-1~M-4 全跳过），直接结束
- `gts-dev-refactor` — **跳过 M 阶段**（M-1~M-4 全跳过），直接结束
